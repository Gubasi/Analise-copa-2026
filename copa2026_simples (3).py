...
# ----------------------------------------
    ...
    st.divider()
    st.subheader("💰 Valores de mercado em destaque")

    jogador_mais_valioso  = dados.loc[dados['VALOR_M'].idxmax()]
    jogador_menos_valioso = dados[dados['VALOR_M'] > 0].loc[dados[dados['VALOR_M'] > 0]['VALOR_M'].idxmin()]

    cv1, cv2, cv3 = st.columns(3)
    cv1.metric(
        "Jogador mais valioso",
        fmt_valor(jogador_mais_valioso['VALOR_M']),
        f"↑ {jogador_mais_valioso['PLAYER NAME']}"
    )
    cv2.metric(
        "Jogador menos valioso",
        fmt_valor(jogador_menos_valioso['VALOR_M']),
        f"↑ {jogador_menos_valioso['PLAYER NAME']}"
    )
    valor_medio = dados[dados['VALOR_M'] > 0]['VALOR_M'].mean()
    cv3.metric("Valor médio por jogador", fmt_valor(valor_medio))
...
# ----------------------------------------
    ...
    with aba1:
        ...
        lc1.metric("Liga com mais convocados",  f"{_liga_mais['Jogadores']} jogadores",  f"↑ {_liga_mais['Liga']}")
        lc2.metric("Liga com menos convocados", f"{_liga_menos['Jogadores']} jogador(es)", f"↑ {_liga_menos['Liga']}")
    ...
    with aba2:
        ...
        cc1.metric("Clube com mais convocados",  f"{_clube_mais['Jogadores']} jogadores",  f"↑ {_clube_mais['Clube']}")
        cc2.metric("Clube com menos convocados", f"{_clube_menos['Jogadores']} jogador(es)", f"↑ {_clube_menos['Clube']}")
