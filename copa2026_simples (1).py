...
if pagina == "🏠 Início":
    ...
    fig = px.bar(por_grupo, x='GRUPO', y='VALOR_M', text='VALOR_M',
                 color='VALOR_M', color_continuous_scale='Blues',
                 labels={'GRUPO': '', 'VALOR_M': 'Valor (€M)'})
...
# ----------------------------------------
    ...
    fig2 = px.bar(por_selecao, x='PAIS', y='VALOR_M', color='GRUPO', text='VALOR_M',
                  labels={'VALOR_M': 'Valor total (€M)', 'PAIS': ''})
...
# ----------------------------------------
    ...
    if modo == "Comparar todos os grupos":
        ...
        for i, (_, row) in enumerate(selecoes.iterrows()):
            with cols[i]:
                st.metric(row['PAIS'], f"€ {row['valor']:.0f}M", f"Média: {row['idade']:.1f} anos")
