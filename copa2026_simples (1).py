...
df = df[df['GRUPO'].notna()].copy()  # remove seleções sem grupo mapeado

# Nome do país sem o código (ex: "France (FRA)" → "France")
df['PAIS'] = df['TEAM'].str.extract(r'^(.*?)\s*\(')

# Formata valor: B se >= 1B, M se >= 1M, K se menor
...
with st.sidebar:
    ...
    grupo_filtro = st.selectbox("Filtrar por grupo", ["Todos"] + sorted(df['GRUPO'].dropna().unique()))
    pos_filtro   = st.selectbox("Filtrar por posição", ["Todas"] + sorted(df['POS'].dropna().unique()))
...
# ----------------------------------------
    ...
    _jmenos_df = dados[dados['VALOR_M'] >= 0.001]   # >= 1K, exclui zeros reais
    _jmenos = _jmenos_df.loc[_jmenos_df['VALOR_M'].idxmin()]
    _vmedio = _jmenos_df['VALOR_M'].mean()
...
# ----------------------------------------
    ...
    if modo == "Comparar todos os grupos":
        ...
        grupo_escolhido = st.selectbox("Escolha o grupo", sorted(df['GRUPO'].dropna().unique()))
